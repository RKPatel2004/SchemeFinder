...
import './AdminEditScheme.css';

const AdminEditScheme = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  const initialSchemeData = location.state?.scheme || null;
  ...
  useEffect(() => {
    if (initialSchemeData) {
      setSchemeData(initialSchemeData);
      setOriginalSchemeData(initialSchemeData);
    } else {
  ...
  useEffect(() => {
    if (!originalSchemeData) return;

    const isDataModified = () => {
      if (schemeData.schemeName !== originalSchemeData.schemeName) return true;
      if (schemeData.description !== originalSchemeData.description) return true;
      
      const criteriaKeys = Object.keys(schemeData.eligibilityCriteria);
      for (let key of criteriaKeys) {
        if (key === 'age') {
          if (
            schemeData.eligibilityCriteria.age.min !== originalSchemeData.eligibilityCriteria.age.min ||
            schemeData.eligibilityCriteria.age.max !== originalSchemeData.eligibilityCriteria.age.max
          ) {
            return true;
          }
        } else {
          if (schemeData.eligibilityCriteria[key] !== originalSchemeData.eligibilityCriteria[key]) {
            return true;
          }
        }
      }

      if (JSON.stringify(schemeData.benefits) !== JSON.stringify(originalSchemeData.benefits)) return true;
      if (JSON.stringify(schemeData.applicationProcess) !== JSON.stringify(originalSchemeData.applicationProcess)) return true;
      if (JSON.stringify(schemeData.requiredDocuments) !== JSON.stringify(originalSchemeData.requiredDocuments)) return true;

      return false;
    };

    setIsModified(isDataModified());
  }, [schemeData, originalSchemeData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/admin/schemes/${originalSchemeData.schemeName}`,
        schemeData,
        {
          headers: {
      ...
      setSuccess('Scheme edited successfully!');
      setOriginalSchemeData(response.data);
      setIsModified(false);
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to edit scheme. Please try again.');
      console.error(err);
    }
  };

  return (
    <div className="admin-edit-scheme">
      {/* Navbar */}
      ...
      {/* Main Content */}
      <div className="edit-scheme-content">
        <h2>Edit Scheme</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Submit Button */}
          <div className="form-group">
            <button type="submit" className="submit-btn" disabled={!isModified}>Edit Scheme</button>
          </div>
        </form>
      </div>
...
export default AdminEditScheme;
